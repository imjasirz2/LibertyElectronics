document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🔽 Mapping of categories and brands
  ...
  let filteredProducts = [...products]; // Full list at start
  ...
  function renderProducts(reset = false) {
    ...
    nextProducts.forEach(product => {
      ...
      col.innerHTML = `
        <div class="card h-100">
          <img src="Assets/img/Products/${product.image}" class="card-img-top" alt="${product.name}">
          <div class="card-body p-2">
            <h6 class="card-title">${product.name}</h6>
            <p class="card-text text-danger fw-bold">Rs. ${Number(product.price).toLocaleString()}</p>
            <a href="ProductsPages/${category}/${brand}/${productUrl}" class="btn btn-sm btn-primary">Discover</a>
          </div>
        </div>`;
    ...
    // Hide Load More if all products loaded or no match
    loadMoreBtn.style.display = currentIndex >= filteredProducts.length ? "none" : "inline-block";
  }
  ...
  function applyFilters() {
    ...
    renderProducts(true); // Reset view on new filter
  }
  ...
  function updateAvailableFilters() {
    ...
    brandCheckboxes.forEach(cb => {
      if (selectedCategories.length === 0) {
        ...
        const allowedBrands = selectedCategories.flatMap(cat => categoryBrandMap[cat] || []);
        cb.parentElement.style.display = allowedBrands.includes(cb.value) ? "" : "none";
      }
    });
    ...
    categoryCheckboxes.forEach(cb => {
      if (selectedBrands.length === 0) {
        ...
        const allowedCategories = selectedBrands.flatMap(brand => brandCategoryMap[brand] || []);
        cb.parentElement.style.display = allowedCategories.includes(cb.value) ? "" : "none";
      }
    });
  }
